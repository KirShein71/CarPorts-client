...
const CreateWarehouseAssortmentProject = (props) => {
  const { show, setShow, setChange, projectId, existingWarehouseDetailIds = [] } = props;
  ...
  const [filteredWarehouseDetails, setFilteredWarehouseDetails] = React.useState([]);

  React.useEffect(() => {
    if (show) {
      getAllActiveWarehouseAssortement().then((data) => {
        setWarehouseDetails(data);

        // Фильтруем детали: оставляем только те, которых еще нет в проекте
        const filtered = data.filter((detail) => !existingWarehouseDetailIds.includes(detail.id));
        setFilteredWarehouseDetails(filtered);
      });
    }
  }, [show, existingWarehouseDetailIds]);
  ...
  const handleAddWarehouseDetail = () => {
    ...
    if (value.warehouse_assortement && value.warehouse_assortement_name && value.quantity) {
      ...
      setFilteredWarehouseDetails((prev) =>
        prev.filter((d) => d.id !== parseInt(value.warehouse_assortement)),
      );
  ...
  const handleRemoveWarehouseDetail = (index) => {
    const removedDetail = selectedWarehouseDetails[index];

    // Возвращаем деталь в список доступных
    const detailToRestore = warehouseDetails.find(
      (d) => d.id === parseInt(removedDetail.warehouse_assortement_id),
    );

    if (detailToRestore && !existingWarehouseDetailIds.includes(detailToRestore.id)) {
      setFilteredWarehouseDetails((prev) => [...prev, detailToRestore]);
    }

    // Удаляем деталь из выбранных
    setSelectedWarehouseDetails((prev) => prev.filter((_, i) => i !== index));
  };

  const handleRemoveAllWarehouseDetails = () => {
    // Возвращаем все детали в список доступных
    const detailsToRestore = selectedWarehouseDetails
      .map((detail) =>
        warehouseDetails.find((d) => d.id === parseInt(detail.warehouse_assortement_id)),
      )
      .filter((detail) => detail && !existingWarehouseDetailIds.includes(detail.id));

    setFilteredWarehouseDetails((prev) => {
      const combined = [...prev, ...detailsToRestore];
      // Убираем дубликаты
      return combined.filter(
        (item, index) => combined.findIndex((d) => d.id === item.id) === index,
      );
    });

    // Очищаем выбранные детали
    setSelectedWarehouseDetails([]);
  };
        ...
        <Form>
          ...
          <Row className="mb-3">
            <Col>
              <Button
                variant="dark"
                onClick={handleAddWarehouseDetail}
                disabled={!value.warehouse_assortement || !value.quantity}>
                Добавить
              </Button>
            </Col>
          </Row>
...
export default CreateWarehouseAssortmentProject;
